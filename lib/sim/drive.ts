...
export function driveRig(rig: any, pose: Pose, time: number, reducedMotion: boolean) {
  ...
  setJoint(rig, JOINT_NAMES[3], lerp(kneeL0, sit.left_knee, fold) + Math.sin(phase) * 0.12 * hop);
  ...
  setJoint(rig, JOINT_NAMES[12], lerp(kneeR0, sit.right_knee, fold) - Math.sin(phase + 0.6) * 0.1 * hop);
  setJoint(rig, JOINT_NAMES[13], lerp(ankleR0, sit.right_ankle, fold));
  setJawOpen(rig, Math.min(1, pose.beak));

  const placer = rig.placer;
  placer.rotation.x = pose.duckRotation[0] - hop * 0.1;
  ...
  placer.position.y = pose.duckPosition[1] + hop * 0.09 - pose.crouch * 0.016;
