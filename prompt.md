...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/quackles |
| Clone | `git clone https://github.com/kvnloo/quackles.git` |
...
1. Clone or open https://github.com/kvnloo/quackles. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
```bash
gh issue list --repo kvnloo/quackles --label claimable --state open
gh pr list --repo kvnloo/quackles --state open
```
...
6. Fail, then pass (`skills/tdd/SKILL.md`). Unit command: `npm test`. Mutation: `npx stryker run` (write `n/a` if that is what the tree says; do not invent a score).
...
8. Open a PR on https://github.com/kvnloo/quackles at `feature_target` (day) or `overnight_target` (overnight) with `.github/PULL_REQUEST_TEMPLATE.md` filled. Bind evidence to `head_revision`. **Do not merge `main` or `dev`.**
